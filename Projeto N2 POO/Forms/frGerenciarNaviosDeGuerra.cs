...
{
    public partial class frGerenciarNaviosDeGuerra : frBase
    {
        private List<VeiculoNavioDeGuerra> veiculosDoTipoCadastrados = new List<VeiculoNavioDeGuerra>();
        private int index = 0;
        public frGerenciarNaviosDeGuerra()
        {
            InitializeComponent();

            foreach (VeiculoBase veiculo in Dados.Veiculos)
                if (veiculo is VeiculoNavioDeGuerra)
                    veiculosDoTipoCadastrados.Add(veiculo as VeiculoNavioDeGuerra);
